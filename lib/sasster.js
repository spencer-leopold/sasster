...
Sasster.prototype.writeResultsToFileOrStream = function(filename, results) {
  var css, sourceMap, stream, streamObj, seconds;
  var self = this;
  ...
  return new Promise(function(resolve, reject) {
    ...
    if (self.streams) {
      css = new Readable;
      css.push(results.css);
      css.push(null);

      if (!self.options.sourceMap) {
        streamObj = { file: path.basename(filename), css: css, time: seconds };
      }
      else {
        sourceMap = new Readable;
        sourceMap.push(results.map);
        sourceMap.push(null);

        streamObj = { file: path.basename(filename), css: css, sourceMap: sourceMap, time: seconds };
      }
